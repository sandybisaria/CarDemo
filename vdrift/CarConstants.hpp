...
static void getWheelPosStr(int i, int numWheels, WheelPosition& wl, WheelPosition& wr, std::string& pos) {
	if (numWheels == 2) {
		if (i == 0) {
			wl = wr = FRONT_LEFT; pos = "front";
		} else if (i == 1) {
			wl = wr = FRONT_RIGHT;  pos = "rear";
		}
	} else {
		if (i == 0) {
			wl = FRONT_LEFT; wr = FRONT_RIGHT; pos = "front";
		} else if (i == 1) {
			wl = REAR_LEFT; wr = REAR_RIGHT; pos = "rear";
		} else if (i == 2) {
			wl = REAR2_LEFT; wr = REAR2_RIGHT; pos = "rear2";
		} else if (i == 3) {
			wl = REAR3_LEFT; wr = REAR3_RIGHT; pos = "rear2";
		}
	}
}
