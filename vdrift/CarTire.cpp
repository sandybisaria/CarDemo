...
void CarTire::findSigmaHatAlphaHat(double load, double& outputSigmaHat,
								   double& outputAlphaHat, int iterations) const {
...
void CarTire::lookUpSigmaHatAlphaHat(double normalForce, double& sh, double& ah) const {
	...
	size_t HAT_ITERATIONS = sigmaHat.size();
	...
	if (nf < HAT_LOAD) {
		...
		lbound--;
		lbound = std::max(lbound, 0);

		double blend = (nf - HAT_LOAD * (lbound + 1)) / HAT_LOAD;
...
MathVector<double, 3> CarTire::getForce(double normalForce, double fricCoeff,
	...
	Fz = std::min(Fz, 30.0); // Cap Fz at a magic number to prevent explosions
	...
	double sigma = (patchSpeed - V) / denom;
	double alpha = -atan2(hubVelocity[1], denom) * 180.0 / M_PI;
	...
	double Mz = PacejkaMz(alpha, Fz, gamma, fricCoeff, maxMz);
	...
	double longfactor = 1.0;
	double combforce = std::abs(Fx) + std::abs(Fy);
	...
	double maxforce = std::abs(maxFx)*longfactor + (1.0 - longfactor) * std::abs(maxFy); // Linear interpolation
...
void CarTire::calculateSigmaHatAlphaHat(size_t tableSize) {
	double HAT_LOAD = 0.5;
...
// Lateral
double CarTire::PacejkaFy(double alpha, double Fz, double gamma, double fricCoeff,
						  double& maxForceOutput) const {
	const std::vector<double>& a = lateral;

	double D = (a[1] * Fz + a[2]) * Fz * fricCoeff;
	double B = a[3] * sin(2.0 * atan(Fz / a[4])) * (1.0 - a[5] * std::abs(gamma)) /
			   (a[0] * (a[1] * Fz + a[2]) * Fz);
	assert(!isnan(B));
	double E = a[6] * Fz + a[7];
...
// Aligning
double CarTire::PacejkaMz(double alpha, double Fz, double gamma, double fricCoeff,
						  double& maxForceOutput) const {
	const std::vector<double>& c = aligning;
